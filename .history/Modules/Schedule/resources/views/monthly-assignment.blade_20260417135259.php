            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="mt-3 d-flex flex-wrap">
                        <a href="{{ route('schedule.index') }}" class="btn btn-outline-light btn-sm mr-2 mb-2">Quay lai danh
                            sach</a>
                            ...
                            <form method="POST"
                                ...
                                <div class="input-group input-group-sm">
                                    <input type="text" class="form-control" name="comment"
                                        placeholder="Ghi chu gui duyet (optional)">
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <p class="small text-muted mt-2 mb-0">Môn học ở bước này được lấy từ lịch tổng quát và chỉ hiển thị
                            các môn của khoa chịu trách nhiệm.</p>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <form method="POST" action="{{ route('monthly-schedule.assignment.save', $monthlySchedule->id) }}">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped table-hover align-middle assignment-table">
                                <thead>
                                    <tr>
                                        <th style="width: 140px;">Ngay / Tiet</th>
                                ...
                                <tbody>
                                        ...
                                        <tr>
                                            <td>
                                                <div class="font-weight-bold">
                                                    {{ optional($slot->date)->format('d/m/Y (D)') ?? '-' }}
                                                </div>
                                                <small class="text-muted">
                                                    Tiết {{ $slot->period_number }}
                                                    @if ($slot->period_number <= 5)
                                                        <span class="badge badge-light-success">Sáng</span>
                                                    @else
                                                        <span class="badge badge-light-warning">Chiều</span>
                                                    @endif
                                                </small>
                                                <input type="hidden" name="slots[{{ $index }}][id]"
                                                    value="{{ $slot->id }}">
                                            </td>
                                            <td>{{ $slot->trainingClass->name ?? '-- Chưa xác định lớp --' }}</td>
                                            <td>
                                                <select name="slots[{{ $index }}][teacher_id]"
                                            ...
                                            <td>
                                                ...
                                                <input type="hidden" name="slots[{{ $index }}][subject_id]"
                                                    value="{{ $selectedSubjectId }}">

                                                <div>{{ $subjectLabel }}</div>
                                                @if ($selectedSubject?->department)
                                                    <small
                                                        class="text-muted">{{ $selectedSubject->department->name }}</small>
                                                @endif
                                            </td>
                                            <td>
                                                @php
                                                    $selectedLesson =
                                                        $oldSlot['subject_lesson_id'] ?? $slot->subject_lesson_id;
                                                    $lessonOptions = $subjectLessons->where(
                                                        'subject_id',
                                                        $selectedSubjectId,
                                                    );
                                                @endphp
                                                <select name="slots[{{ $index }}][subject_lesson_id]"
                                                    class="form-control form-control-sm">
                                                    ...
                                                    @foreach ($lessonOptions as $lesson)
                                                        <option value="{{ $lesson->id }}" @selected((int) $selectedLesson === (int) $lesson->id)>
                                                            S{{ $lesson->subject_id }}-B{{ $lesson->lesson_no }}:
                                                            {{ $lesson->title }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <td>
                                                <select name="slots[{{ $index }}][room_id]"
                                                    class="form-control form-control-sm">
                                            ...
                                            <td>
                                                <input type="text" class="form-control form-control-sm"
                                                    ...
                                                    value="{{ $oldSlot['content'] ?? $slot->content }}" maxlength="500"
                                                    placeholder="Noi dung tiet hoc">
                                            </td>
                                            <td>
                                                <input type="text" class="form-control form-control-sm"
                                                    name="slots[{{ $index }}][note]"
                                                    value="{{ $oldSlot['note'] ?? $slot->note }}" maxlength="500"
                                                    placeholder="Ghi chu">
                                            </td>
                                            <td>
                                                @php
                                                    $selectedStatus =
                                                        $oldSlot['slot_status'] ?? ($slot->slot_status ?? 'planned');
                                                @endphp
                                                <select name="slots[{{ $index }}][slot_status]"
                                                    class="form-control form-control-sm">
                                                    ...
                                                    <option value="cancelled" @selected($selectedStatus === 'cancelled')>cancelled
                                                    </option>
                                                </select>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="9" class="text-center text-muted py-4">Khong co tiet hoc nao de
                                                phan cong.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                        ...
                        <div class="mt-3 d-flex flex-wrap">
                            ...
                            @if ($canSubmitToTrainingOffice)
                                <small class="text-muted align-self-center mb-2">Sau khi luu, bam "Gui PDT duyet" o tren de
                                    trinh lich thang.</small>
                            @endif
                        </div>
                    </form>
                </div>
