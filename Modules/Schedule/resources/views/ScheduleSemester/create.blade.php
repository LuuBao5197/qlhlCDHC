...
@section('content')
    ...
    <style>
        .box { border: 1px solid #dbe7f3; border-radius: 12px; background: #fff; }
        .box-head { padding: 18px 20px; background: linear-gradient(135deg, #0f4c81, #1677b3); color: #fff; }
        .box-body { padding: 20px; }
        .class-list { max-height: 260px; overflow-y: auto; border: 1px solid #dbe7f3; border-radius: 10px; padding: 12px; }
        .class-pane { display: none; border: 1px solid #dbe7f3; border-radius: 12px; padding: 16px; background: #fff; }
        .class-pane.active { display: block; }
        .rule-card { border: 1px solid #dfe8f1; border-radius: 10px; padding: 14px; background: #f8fbff; }
        .rule-card.has-error { border-color: #dc3545; background: #fff5f5; }
        .weekday-list { display: flex; flex-wrap: wrap; gap: 8px; }
        .weekday-list label { margin-bottom: 0; padding: 6px 10px; border: 1px solid #dbe7f3; border-radius: 999px; background: #fff; }
    </style>
            ...
            <div class="box shadow-sm">
                ...
                <div class="box-body">
                    ...
                    <form action="{{ route('schedule.store') }}" method="POST" enctype="multipart/form-data" id="scheduleForm">
                        ...
                        <div class="row mt-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" name="default_subject" class="form-control" list="subject-suggestions"
                                    value="{{ old('default_subject', 'Chua xep mon') }}">
                            </div>
                            <div class="col-md-6">
                        ...
                        <div class="row">
                            <div class="col-lg-7">
                                <input type="text" class="form-control mb-3" id="classFilterInput" placeholder="Tim theo ma hoac ten lop">
                                <div class="class-list">
                                    @foreach ($classes as $class)
                                        <div class="class-item mb-2" data-search="{{ strtolower($class->code . ' ' . $class->name) }}">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const labelOf = (k) => {
                ...
                if (String(k).startsWith('name:')) return `${extraInput.value.trim() || String(k).slice(5)} (bo sung)`;
                return String(k);
            };

            const activate = (k) => {
                tabs.querySelectorAll('[data-key]').forEach((btn) => {
                    btn.className = `btn btn-sm mr-2 mb-2 ${btn.dataset.key === String(k) ? 'btn-primary' : 'btn-outline-primary'}`;
                });
                content.querySelectorAll('.class-pane').forEach((pane) => pane.classList.toggle('active', pane.dataset.key === String(k)));
            };
            ...
            const parseRule = (card) => {
                ...
                const weekdays = Array.from(card.querySelectorAll('input[name*="[weekdays]"]:checked')).map((input) => Number(input.value));
                ...
                return { startDate, endDate, periodFrom, periodTo, weekdays };
            };
                ...
                content.querySelectorAll('.class-pane').forEach((pane) => {
                    ...
                    pane.querySelectorAll('.rule-card').forEach((card) => {
                        ...
                        for (const cursor = new Date(start); cursor <= end; cursor.setDate(cursor.getDate() + 1)) {
                            ...
                            for (let period = parsed.periodFrom; period <= parsed.periodTo; period++) {
                                const slotKey = `${dateKey}|${period}`;
                                if (slotOwners.has(slotKey)) {
                                    hasConflict = true;
                                    appendRuleError(card, `Trung lich voi rule khac trong ${classLabel} tai ${dateKey}, tiet ${period}.`);
                                    appendRuleError(slotOwners.get(slotKey), `Trung lich voi rule khac trong ${classLabel} tai ${dateKey}, tiet ${period}.`);
                                } else {
            ...
            const weekdayHtml = (k, i, picked) => weekdayOptions.map((d) => {
                const checked = Array.isArray(picked) && picked.map(String).includes(String(d.value)) ? 'checked' : '';
            ...
            filter.addEventListener('input', function() {
                ...
                document.querySelectorAll('.class-item').forEach((item) => item.style.display = item.dataset.search.includes(q) ? '' : 'none');
            });
