...
@section('title', 'Nhật Ký Trực Ban Huấn Luyện')
...
<div class="nhatky-wrapper">
    <div class="nhatky-toolbar">
        <form method="GET" action="{{ route('duty-log.index') }}" class="d-flex align-items-center gap-2" style="gap: 8px;">
        ...
        <button type="button" class="btn-nhatky-print" onclick="window.print()">In nhật ký</button>
    </div>

    <div class="nhatky-alert-info">
        Trang này chỉ dùng để xem dữ liệu đã đổ ra. Chỉnh sửa và lưu chỉ thực hiện ở trang chỉnh sửa.
    </div>
    ...
    <div class="nhatky-card">
        ...
        <div class="nhatky-section-title">1. Quân số, nội dung huấn luyện</div>
        ...
        <div class="nhatky-section2">
            <div class="item">
                ...
                <div class="item-content">{{ $dailySummary?->training_plan_comment ?: '—' }}</div>
            </div>

            <div class="item">
                <div class="item-label">b) Thực hiện quy chế, quy định về GDĐT</div>
                <div class="item-content">{{ $dailySummary?->regulation_comment ?: '—' }}</div>
            </div>

            <div class="item">
                <div class="item-label">c) Quản lý, sử dụng hội trường, vật chất, trang thiết bị đào tạo</div>
                <div class="item-content">{{ $dailySummary?->facility_comment ?: '—' }}</div>
            </div>

            <div class="item">
                <div class="item-label">d) Những việc cần tiếp tục xử lý</div>
                <div class="item-content">{{ $dailySummary?->followup_comment ?: '—' }}</div>
            </div>
        </div>
