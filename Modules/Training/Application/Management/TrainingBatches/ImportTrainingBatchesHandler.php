...
class ImportTrainingBatchesHandler
{
    private const MAX_ROWS = 1000;

    private const ALLOWED_STATUSES = ['active', 'inactive', 'archived'];

    public function handle(ImportTrainingBatchesRequest $request): JsonResponse
    {
        ...
        DB::transaction(function () use ($records): void {
            foreach (array_chunk($records, 500) as $chunk) {
                TrainingBatch::query()->insert($chunk);
            }
        });

        return response()->json([
            'message' => 'Nhập danh sách khóa học thành công.',
    ...
    private function readRows(UploadedFile $file): array
    {
        ...
        try {
            ...
            $programsByCode = TrainingProgram::query()
                ->get(['id', 'code'])
                ->keyBy(fn (TrainingProgram $program): string => mb_strtolower($program->code, 'UTF-8'));
            ...
            while (($values = fgetcsv($handle, 0, $delimiter, '"', '')) !== false) {
                ...
                $rowErrors = $this->validateRow($row, $lineNumber, $seenCodes, $programsByCode);
                ...
                $program = $programsByCode->get(mb_strtolower($row['training_program_code'], 'UTF-8'));
            ...
            if ($dataRowCount === 0) {
                $errors[] = 'File CSV không có dòng khóa học nào.';
            }
    ...
    private function validateHeaders(array $headerMap, array $headers): array
    {
        ...
        foreach (['training_program_code', 'code', 'name'] as $requiredHeader) {
    ...
    private function normalizeRow(array $values, array $headerMap): array
    {
        ...
        foreach (['training_program_code', 'code', 'name', 'status'] as $column) {
     ...
     * @param \Illuminate\Support\Collection<string, TrainingProgram> $programsByCode
     * @return array<int, string>
     */
    private function validateRow(array $row, int $lineNumber, array $seenCodes, $programsByCode): array
    {
        $errors = [];
        ...
        } elseif (!$programsByCode->has(mb_strtolower($row['training_program_code'], 'UTF-8'))) {
        ...
        if ($row['code'] === '') {
            ...
            if (array_key_exists($normalizedCode, $seenCodes)) {
                $errors[] = "Dòng {$lineNumber}: mã khóa học '{$row['code']}' bị trùng với dòng {$seenCodes[$normalizedCode]}.";
            }
        }
    ...
    private function findExistingErrors(array $rows): array
    {
        ...
        foreach ($existingCodes as $code => $_) {
            $row = $rowsByCode[$code];
            $errors[] = "Dòng {$row['line_number']}: mã khóa học '{$row['code']}' đã tồn tại.";
        }
