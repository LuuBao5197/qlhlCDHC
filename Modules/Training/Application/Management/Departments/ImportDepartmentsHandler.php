...
class ImportDepartmentsHandler
{
    private const MAX_ROWS = 1000;

    private const ALLOWED_STATUSES = ['active', 'inactive'];

    public function handle(ImportDepartmentsRequest $request): JsonResponse
    {
        ...
        DB::transaction(function () use ($records): void {
            foreach (array_chunk($records, 500) as $chunk) {
                Department::query()->insert($chunk);
            }
        });

        return response()->json([
            'message' => 'Nhập danh sách khoa/bộ môn thành công.',
    ...
    private function readRows(UploadedFile $file): array
    {
        ...
        try {
            ...
            while (($values = fgetcsv($handle, 0, $delimiter, '"', '')) !== false) {
                ...
                $rowErrors = $this->validateRow($row, $lineNumber, $seenCodes, $seenNames);
            ...
            if ($dataRowCount === 0) {
                $errors[] = 'File CSV không có dòng khoa/bộ môn nào.';
            }
    ...
    private function normalizeRow(array $values, array $headerMap): array
    {
        ...
        foreach (['code', 'name', 'description', 'status'] as $column) {
            $value = array_key_exists($column, $headerMap)
                ? (string) ($values[$headerMap[$column]] ?? '')
                : '';
            $value = trim($value);
            $row[$column] = $column === 'description'
                ? $value
     ...
     * @return array<int, string>
     */
    private function validateRow(array $row, int $lineNumber, array $seenCodes, array $seenNames): array
    {
        $errors = [];
        ...
        if ($row['code'] === '') {
            ...
            if (array_key_exists($normalizedCode, $seenCodes)) {
                $errors[] = "Dòng {$lineNumber}: mã khoa '{$row['code']}' bị trùng với dòng {$seenCodes[$normalizedCode]}.";
            }
        }
        ...
        if ($row['name'] === '') {
            ...
            if (array_key_exists($normalizedName, $seenNames)) {
                $errors[] = "Dòng {$lineNumber}: tên khoa '{$row['name']}' bị trùng với dòng {$seenNames[$normalizedName]}.";
            }
        }
    ...
    private function findExistingErrors(array $rows): array
    {
        ...
        foreach ($existingCodes as $code => $_) {
            $row = $rowsByCode[$code];
            $errors[] = "Dòng {$row['line_number']}: mã khoa '{$row['code']}' đã tồn tại.";
        }

        foreach ($existingNames as $name => $_) {
            $row = $rowsByName[$name];
            $message = "Dòng {$row['line_number']}: tên khoa '{$row['name']}' đã tồn tại.";
