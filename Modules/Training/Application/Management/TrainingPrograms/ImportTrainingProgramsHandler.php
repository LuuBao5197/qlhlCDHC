...
class ImportTrainingProgramsHandler
{
    private const MAX_ROWS = 1000;

    private const ALLOWED_STATUSES = ['active', 'inactive', 'archived'];

    public function handle(ImportTrainingProgramsRequest $request): JsonResponse
    {
        ...
        DB::transaction(function () use ($records): void {
            foreach (array_chunk($records, 500) as $chunk) {
                TrainingProgram::query()->insert($chunk);
            }
        });

        return response()->json([
            'message' => 'Nhập danh sách chương trình đào tạo thành công.',
    ...
    private function readRows(UploadedFile $file): array
    {
        ...
        try {
            ...
            if ($dataRowCount === 0) {
                $errors[] = 'File CSV không có dòng chương trình đào tạo nào.';
            }
    ...
    private function normalizeRow(array $values, array $headerMap): array
    {
        ...
        foreach (['code', 'name', 'status'] as $column) {
    ...
    private function validateRow(array $row, int $lineNumber, array $seenCodes): array
    {
        ...
        if ($row['code'] === '') {
            ...
            if (array_key_exists($normalizedCode, $seenCodes)) {
                $errors[] = "Dòng {$lineNumber}: mã chương trình '{$row['code']}' bị trùng với dòng {$seenCodes[$normalizedCode]}.";
            }
        }
    ...
    private function findExistingErrors(array $rows): array
    {
        ...
        foreach ($existingCodes as $code => $_) {
            $row = $rowsByCode[$code];
            $errors[] = "Dòng {$row['line_number']}: mã chương trình '{$row['code']}' đã tồn tại.";
        }
